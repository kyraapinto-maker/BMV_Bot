...
export default function Sourcing() {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    setIsSearching(true);
    try {
      const res = await fetch(buildUrl(api.properties.search.path) + `?postcode=${encodeURIComponent(postcode)}`);
      ...
      {results.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {results.map((prop, idx) => (
            <Card key={idx} className="overflow-hidden border-border/50 hover-elevate transition-all duration-300">
              <CardHeader className="bg-muted/30 pb-3">
                <div className="flex justify-between items-start gap-2">
                  <Badge variant="secondary" className="bg-primary/10 text-primary border-none font-semibold">
                    Potential
                  </Badge>
                  <span className="text-xl font-bold text-foreground">£{prop.price?.toLocaleString()}</span>
                </div>
